...
for var in vars.keys():
    ...
    for function in functions.keys():
        ...
        plt.title("Overfitting " + functions[function] + " " + vars[var])
        plt.xlabel("Epochs[#]")
        plt.ylabel("CIFAR10\n" + vars[var] + " [%]")
        for s in sparsities.keys():
            ...
            plt.plot(data*100-data2*100,colors[color_index],label= sparsities[s])
            color_index += 1
        ...
        #     plt.ylim([0, 200])
        plt.tight_layout()
        plt.savefig("results/cifar10/overfitting_" + var + "_" + function + ".pdf")
        plt.close()

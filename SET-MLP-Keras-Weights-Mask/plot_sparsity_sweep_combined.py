...
functions = {
    "relu": "ReLU", "sigmoid": "Sigmoid", "tanh": "Tanh", "selu": "SELU", "softsign": "Softsign", "softplus": "Softplus", "srelu": "SReLU"}
for var in vars.keys():
    ...
    for function in functions.keys():
        ...
        l = functions[function] + " " + vars[var]
        set_suffix = " on training set"
        plt.plot(x, sparsity_sweep[function], '--' + colors[color_index], label=l + set_suffix)
        # color_index += 1
        dense_l = functions[function] + " Dense " + vars[var]
        ...
        sparsity_sweep_val[function] = [eps10[-1] * 100, eps20[-1] * 100, eps50[-1] * 100, eps100[-1] * 100,
                                    eps500[-1] * 100]

        plt.plot(x, sparsity_sweep_val[function], colors[color_index], label=l + set_suffix)

        plt.plot(x, [dense[-1]*100]*5, ':' + colors[color_index], label=dense_l + set_suffix)
        color_index += 1

    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0., prop={'size': 4})
    ...
    if var == "acc" or var == "val_acc":
        ...
        plt.ylim([0, 200])
    plt.tight_layout()
    plt.savefig("results/cifar10/sparsity_sweep_" + var + "_combined.pdf")
    plt.close()
